...
public class PagamentosPendentesBean implements Serializable {
	...
	public void fecharPagamento(){
		...
		try {
			...
			Boolean compraPendente = false;

			List<Pagamento> pagamentos = pagamentoDAO.getPagamentosPendentesPorCompra(pagamento.getCompra());

			if (pagamentos.size() > 0)
				compraPendente = true;

			if (!compraPendente){
				pagamento.getCompra().setStatusCompra(StatusCompra.PAGO);
				compraDAO.save(pagamento.getCompra());
			}
